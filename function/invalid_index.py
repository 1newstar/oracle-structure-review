...
=====规则配置文件=====
[rule1]
name        = INVALID_INDEX                     #规则名称
description = 最近7天未使用过的索引             #规则描述
weight   = 0.1                               #扣分阀值
max_value   = 10                                #扣分上限
status      = ON/OFF                            #规则状态
title1      = 表名称                            #返回数据标题1
...
************************************************************"""
def f_rule_invalid_index(p_parms):
					...
					SELECT T.TABLE_NAME,T.INDEX_NAME
					 ...
					 WHERE T.OWNER = '"""+l_username+"""'
					   AND NOT EXISTS (SELECT 1
					          FROM V$SQL_PLAN S
					         WHERE S.OBJECT_NAME = T.INDEX_NAME
					           AND S.OBJECT_OWNER = '"""+l_username+"""')
					INTERSECT
					SELECT T.TABLE_NAME,T.INDEX_NAME
					  FROM DBA_INDEXES T
					 WHERE T.OWNER = '"""+l_username+"""'
					   AND NOT EXISTS (SELECT 1
					         ...
					         WHERE P.OBJECT_NAME = T.INDEX_NAME
					           AND P.OBJECT_OWNER = '"""+l_username+"""')""")
