...
返回值
    ...
    'scores'    float   扣分
    'records'   list    dblink信息(dblink的名,访问的用户，访问的主机)
示例
    v_dbinfo=["localhost","1521","xe","hf","123"]
    v_parms=[v_dbinfo,'HF',1,10]
...
************************************************************"""
def f_rule_dblinks_num(p_parms):
    [l_dbinfo,l_owner,l_weight,l_max_value]=p_parms
    l_return_stru={"scores":0,"records":[]}
    conn=cx_Oracle.connect(l_dbinfo[3]+'/'+l_dbinfo[4]+'@'+l_dbinfo[0]+':'+l_dbinfo[1]+'/'+l_dbinfo[2])
    cursor = conn.cursor()
    cursor.execute("""select db_link,username,host from dba_db_links where owner='"""+l_owner+"""'""")
